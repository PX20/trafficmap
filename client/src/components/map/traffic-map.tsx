  ...
  const createCustomMarker = (markerType: string, color: string) => {
    const getIconSvg = (type: string) => {
      switch(type.toLowerCase()) {
        ...
        default:
          // For traffic events (crash, hazard, restriction) - just use colored circles
          return `<div style="background-color: ${color}; width: 12px; height: 12px; border-radius: 50%;"></div>`;
      }
    };
